...
@then('migration info in schema "{schema}" contains forced baseline={baseline}')
def step_impl(context, schema, baseline):
    cur = context.conn.cursor()
    info = _get_info(context.migr_dir, 0, 1, cur, schema=schema)
    assert list(info.values())[0]['version'] == int(baseline)
    assert list(info.values())[0]['description'] == 'Forced baseline'
